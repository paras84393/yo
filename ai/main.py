...
st.title("For Chavi ✨")
st.write("Please click 'Start' below and allow camera permissions to begin.")

# Base resolution for processing grid
...
# WebRTC Ice Servers configuration (Crucial for remote cross-network streaming)
...
class WebLoveEngine:
    ...
    def process_frame(self, frame):
        ...
        if self.state in ["INTRO_1", "INTRO_2", "INTRO_3"]:
            ...
            if elapsed > 5.0:
                self.state = "CAMERA_REVEAL" if self.state != "INTRO_3" else "CAMERA_REVEAL"
                if self.state == "CAMERA_REVEAL": self.state = "CAMERA_REVEAL"
                self.state = "INTRO_2" if self.state == "INTRO_1" else ("INTRO_3" if self.state == "INTRO_2" else "CAMERA_REVEAL")
                self.start_time = time.time()
            return out_frame
        ...
        if self.state == "CAMERA_REVEAL":
            ...
            if self.frame_count % 12 == 0: self.spawn_heart(random.randint(50, WIDTH-50), -20, 0.6)
            if elapsed < 4.0:
        ...
        # --- SCENE 5: THE PROPOSAL ---
            ...
            if results.multi_hand_landmarks:
                for hl in results.multi_hand_landmarks:
                    ...
                    if elapsed > 3.0 and self.is_fist(hl):
                        for _ in range(40): self.spawn_heart(int(tx), int(ty), random.uniform(0.5, 1.2), True)
            ...
            if self.frame_count % 5 == 0: self.spawn_heart(random.randint(20, WIDTH-20), -20, random.uniform(0.5, 1.0))
        ...
        # --- SCENE 6: GRAND FINALE (INTERACTIVE MULTI-FIST POPS) ---
            ...
            if self.frame_count % 3 == 0: self.spawn_heart(random.randint(0, WIDTH), HEIGHT + 20, random.uniform(0.6, 1.5), True)
...
# Persistent session state for engine caching across context re-runs
if "engine" not in st.session_state:
    st.session_state.engine = WebLoveEngine()

def video_frame_callback(frame):
    img = frame.to_ndarray(format="bgr24") # Convert input buffer to numpy matrix
    img_flipped = cv2.flip(img, 1) # Mirror image for easy interaction
...
# Instantiate WebRTC component mapping browser feed to callback pipeline
webrtc_streamer(
